...
module('Integration | Helper | if-key', function (hooks) {
  ...
  module('error cases', function (hooks) {
      ...
      await render(
        <template>
          <div
            id="if-key-target"
            tabindex="0"
            {{on "keydown" (ifKey "alt+c" unknownEvent)}}
          ></div>
        </template>,
      );
      await triggerEvent('#if-key-target', 'keydown', {
        altKey: true,
        key: 'c',
      });
    });
    ...
    skip('warns if called without a keyboard event', async function (assert) {
      ...
      await render(
        <template>
          <button type="button" {{on "click" (ifKey "alt+c" onTrigger)}}>Press
            me</button>
        </template>,
      );
      await click('button');
    ...
    await render(
      <template>
        <div
          id="if-key-target"
          tabindex="0"
          {{on "keydown" (ifKey "alt+c" onTrigger)}}
        ></div>
      </template>,
    );
    await triggerEvent('#if-key-target', 'keydown', { altKey: true, key: 'c' });
